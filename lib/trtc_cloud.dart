...
class TRTCCloud {
  ...
  /// Parameters:
  ///
  /// config For more information, please see the description of `TRTCTranscodingConfig` in `trtc_cloud.def.dart`. If `null` is passed in, On-Cloud MixTranscoding will be canceled.
  ...
  /// Parameters:
  ///
  /// renderParams: rendering parameters (fill mode, rotation angle, mirror mode, etc.). For more information, please see the definition of the `TRTCRenderParams` parameter in `trtc_cloud.def.dart`
  ...
  ///
  /// After this feature is enabled, the evaluation result of the volume by the SDK will be obtained in `onUserVoiceVolume`. To enable this feature, call this API before calling `startLocalAudio()`.
  ///
  /// Parameters:
  ///
  ...
  /// Parameters:
  ///
  /// level For more information, please see `TRTC_LOG_LEVEL`. Default value: TRTCCloudDef.TRTC_LOG_LEVEL_NULL
  ...
  ///
  /// Parameters: jsonStr JSON string of API and parameter descriptions
  ...
  Future<int?> enableCustomVideoProcess(bool enable) async {
    return _channel.invokeMethod('enableCustomVideoProcess', {
      "enable": enable
    });
  }
}
